...
export async function createServer(
  ...
  app.get("/api/pages", express.json(), async (req, res) => {
    try {
      const session = await Shopify.Utils.loadCurrentSession(req, res);
      const page = await Page.all({
        session: session,
      });
  ...
  app.delete("/api/pages/", express.json(), async (req, res) => {
    try {
      const session = await Shopify.Utils.loadCurrentSession(req, res);
      const pageDelArr = req.body.map((id) =>
        Page.delete({ session: session, id: parseInt(id) })
      );
      await Promise.all(pageDelArr);
      res.status(200).json({ success: true });
    } catch (error) {
      console.log(error);
    }
  ...
  app.delete("/api/pages/:id", express.json(), async (req, res) => {
    try {
      const test_session = await Shopify.Utils.loadCurrentSession(req, res);
      await Page.delete({
